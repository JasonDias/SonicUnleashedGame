package prj.sonicunleashed.collidable 
{
	...
	public class SonicFist extends AbstractMapAsset implements IMapAsset, ICollidable
	{		
		private var ourCollisionManager:ICollisionManager;
		
		function SonicFist(myActionFactory:IActionFactory, myWorld:IWorld, myX:Number = 0, myY:Number = 0, myRotation:Number = 0) {
		...
		public function collisionOccurred(myCollidable:ICollidable):void {
			//Debug.log(this+' fist hit enemy ' + myCollidable);
			
			var myCollidedAsset:IMapAsset = myCollidable as IMapAsset;
			if(!myCollidedAsset)
				return;
			
			if(!myCollidedAsset.getDisplay().parent)
				return;
			
			ourDispatcher.dispatchEvent(new SonicFistEvent(SonicFistEvent.HIT_ENEMY));
			ActionsFacade.getInstance().addActionAndStart(PlayerCharacterActions.ATTACK_SUCCESS_ACTION, myCollidedAsset);
