package prj.sonicunleashed.assets 
{
	...
	public class LTowerAsset extends AbstractMapAsset implements IMapAsset 
	{
		public static const TOWER_BITMAP_CLASS_NAME:String	= 'LibAssetLTowerBitmap';
		
		private var ourBitmap:Bitmap;
		
		public function LTowerAsset(myWorld:IWorld, myViewport:IMapAssetViewport, myX:Number=0, myY:Number=0, myRotation:Number=0) {
			super();
			ourWorld = myWorld;
			
			var myBitmapClass:Class			= getDefinitionByName(TOWER_BITMAP_CLASS_NAME) as Class; 
			var myBMPData:BitmapData		= new myBitmapClass(448, 384) as BitmapData;
		...
		public function createBody(myWorld:IWorld, myX:Number = 0, myY:Number = 0, myRotation:Number = 0):void {
			...
			myShapeDef.vertices[1].Set(-hx, hy-myWorldUnits.getMetersFromPixels(128));
			myShapeDef.vertices[2].Set(hx-myWorldUnits.getMetersFromPixels(128), hy-myWorldUnits.getMetersFromPixels(128));
			myShapeDef.vertices[3].Set(hx-myWorldUnits.getMetersFromPixels(128),  hy);
			...
			myShapeDef.vertexCount			= 4;
			myShapeDef.vertices[0].Set(hx-myWorldUnits.getMetersFromPixels(128),  hy);
			myShapeDef.vertices[1].Set(hx-myWorldUnits.getMetersFromPixels(128),  -hy);
